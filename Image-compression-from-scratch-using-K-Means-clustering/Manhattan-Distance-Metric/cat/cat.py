...
cat_im = io.imread('cat.bmp')
io.imshow(cat_im)
io.show()

# size of the image
im_size = cat_im.shape
print(im_size)
...
# Reshape the image
X = cat_im.reshape(cat_im.shape[0]*cat_im.shape[1],3)
print(X.shape)
...
for K in num_clusters:
   ...
   # Recovering compressed image
    ...
    imageio.imsave(f'cat_compressed_{K}.bmp', X_compressed)
    
    # Displaying compressed images
    im = imageio.imread(f'cat_compressed_{K}.bmp')
    io.imshow(im)
    io.show() 
